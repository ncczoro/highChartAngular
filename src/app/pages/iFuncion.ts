export class iFunction {
    ...
    public getStorage(name: string): any {
        if (typeof (Storage) !== 'undefined') {
            if (this._storage.getItem(name)) {
                ...
                console.log("Không tồn tại key tên " + name);
                return "";
            }
        }
        else {
            console.log("Không tồn tại Storage");
            return "";
        }
    }
    ...
    //đảo chọn trên lưới
    public CheckReverse(lstSo: any[], arrSo: any[]) {
        for (let x of lstSo) {
            if (arrSo.length > 0) {
    ...
    public getUserInformation(): any {
        try {
            ...
            console.log(e);
            let strReturn = [{ "SUBDIVISIONID": "PC06AA", "SUBDIVISIONNAME": "", "SUBDIVISIONLEVEL": -1, "SUBDIVISIONID_U": "", "SUBDIVISIONNAME_U": "", "SUBDIVISIONLEVEL_U": -1, "USERID": 1, "USERNAME": "daotao", "PASSWORD": "****", "FULLNAME": "hoacua", "EMAIL": "hoacuat@yahoo.com.vn", "PHONE": "0422100106", "MOBILE": " ", "STATE": 0, "ISUSED": 2, "DEPT": "HNPC", "SESSIONID": "123456789" }];
            return strReturn;
        }
    }
    public getSystemDate(): any[] {
        try {
    ...
    public getSystemParam(): any[] {
        try {
            ...
            console.log(e);
            let strReturn = [{ "NAME": "G_PHANHE_CHOTSL", "VALUE": "BC;BT;CN;HD;KH;TT;CD", "PHAN_HE": "AL" }, { "NAME": "G_INCHUNGRIENG", "VALUE": "R", "PHAN_HE": "HD" }];
            return strReturn;
        }
    }
    public getMenuInfo(): any[] {
        try {
    ...
    public DocSoThanhChu(inumber: string): string {
        let strReturn = "";
        let s = inumber;
        while (s.length > 0 &&
            s.substring(0, 1) == "0") {
    ...
    public ConvertCurrencyToEnglish(inumber: string): string {
        let Temp: any;
    ...
    public ConvertDollarsToVN(inumber: string): string {
        let Temp: any;
        let Dollars = "", Cents: string = "";
        let DecimalPlace, Count: any;
        let Place: any[] = [];
    ...
    /**Xu ly object */
    mergeObject(object1: Object, object2: Object) {
        return { ...object1, ...object2}
    }

    addProperty(obj: Object, field: any, value: any) {
        return { ...obj, field: value}
    }

    deleteProperty(field: any, obj: Object) {
        let newObj = ({field, ...rest}) => rest;
        return newObj
    }

    removeProperty(x) {
        let a  = prop => ({ [prop]: _, ...rest }) => rest
        //                     ----       ------
        //                          \   /
